...
const FAQ_DATA = [
  ...
  { q: 'Can I charge a delivery fee based on distance?', a: 'Yes. Delivery fees are configurable per location. Choose no fee, a flat fee, a distance-based fee with a free radius, or a fully tiered structure — different rates for different distance bands (for example free within 10 miles, a flat fee out to 20, then a per-mile rate beyond that). Distance is estimated from the customer address, and the fee is shown to the customer before they confirm.' },
  { q: 'Can I offer add-on services like hang-dry or hypoallergenic detergent?', a: 'Yes. Each location can define its own catalog of add-ons / processing extras — hang-dry, extra rinse, fabric softener, hypoallergenic detergent, and more — priced per item or per pound. Customers pick them while ordering, and per-pound add-ons are finalized against the weighed amount.' },
  { q: 'Can customers leave tips, and can I set it up the way I want?', a: 'Yes. Tipping is optional and configurable. Customers can tip by percentage or a fixed amount at checkout, on recurring orders, or on an existing order before it is finalized. Tips flow through to your own payment account.' },
  { q: 'How much of my public site can I customize myself?', a: 'A lot, all self-service from the admin: your logo, theme/accent color, homepage promo banner, service list and prices, store hours, hero text, and which navigation and storefront sections show. You can run pickup-and-delivery-only and hide storefront-oriented content without touching code.' },
  { q: 'What happens when someone in an unserved area wants service?', a: 'The platform captures it as demand. If a customer tries to book from outside your service area, their location is recorded so you can see where interest is building, and you can add a "bring service to my area" form to your marketing site that feeds the same list. It helps you decide where to expand next.' },
  { q: 'How does the AI garment tracking work?', a: 'AI-assisted item counting is an optional add-on that records an itemized count of a customer\'s garments before and after washing to reduce lost or mismatched items. It is entirely optional — you can run a standard weigh-and-tag workflow instead, and both are fully supported.' },
];
