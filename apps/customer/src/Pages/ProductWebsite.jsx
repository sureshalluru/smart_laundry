        ...
        <Container maxW="700px">
          ...
          <Accordion allowMultiple>
            {[
              ...
              { q: 'Do we have to offer pickup & delivery, or can we run standard in-store drop-off?', a: 'Pickup & delivery is optional. You can run entirely on walk-in / in-store drop-off using the Quick POS — an in-store order needs only the items, no address or scheduling. Delivery is opt-in per order and defaults to your own driver. Third-party delivery (Uber) is an optional add-on that only turns on if you connect your own Uber account.' },
              { q: 'Do we have to link a website to take payments?', a: 'No. In-store payments are handled at the POS independently — Cash, Card, Terminal, and Pay Later. Cash and Pay Later work with zero payment setup; Card and Terminal just need you to connect your own Stripe account (per location). With no card processor connected, the system runs cash / invoice-only. The customer website is simply an optional online-ordering channel, separate from how you collect payment.' },
            ].map((item, i) => (
