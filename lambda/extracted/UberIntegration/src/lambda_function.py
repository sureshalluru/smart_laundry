...
import os
import re
from db_helpers import (get_laundry_credentials, get_order, update_order_status,
                        update_order_service_field, get_customer, get_laundry_shop,
                        get_frequency_by_address, update_frequency_uber_flags)
import db
...
def get_laundry_credentials(laundry_id, uber_env=None):
    """Fetch Uber credentials from PostgreSQL."""
    cur = db.get_cursor()
    if not uber_env:
        cur.execute("SELECT stripe_terminal_id FROM shop.laundry_shops WHERE laundry_id = %s", (laundry_id,))
        # get uber_env from laundry_uber_credentials — pick first available
        cur.execute("""
            SELECT env FROM shop.laundry_uber_credentials
            WHERE laundry_id = %s ORDER BY env LIMIT 1
        """, (laundry_id,))
        row = cur.fetchone()
        uber_env = row["env"] if row else "test"
...
def handle_uber_webhook(event):
    try:
        ...
        cur = db.get_cursor()
        cur.execute("""
            SELECT order_id, order_status, laundry_id, customer_id,
                   uber_info
        ...
        # Initialize section if missing — store uber_info as JSONB in PostgreSQL
        ...
        # Apply status updates to the in-memory dict
        ...
        # Persist updated uber_info JSONB back to PostgreSQL
        import json as _json
        cur = db.get_cursor()
        cur.execute("""
            UPDATE orders.orders SET uber_info = %s, updated_at = NOW()
            WHERE order_id = %s
        """, (_json.dumps(uber_info), external_id))
        db.commit()
        ...
        if update_target == "laundryPickup" and delivery_status == "delivered":
            if order_item.get("order_status") == "OrderSubmitted":
            ...
            if order_item.get("order_status") == "EnRouteToDelivery":
...
def send_order_status_notification(order_id, order_status):
    try:
        cur = db.get_cursor()
        cur.execute("""
            SELECT o.order_id, o.customer_id, o.laundry_id,
                   c.notif_email, c.notif_sms, c.email, c.phone_number,
                   c.first_name, c.last_name,
                   ls.contact_email AS sender_email
        ...
        if row["notif_email"] and row["email"]:
            invoke_notification_lambda("email", row["email"], message,
                                       "Order Status Update", row["sender_email"])
        if row["notif_sms"] and row["phone_number"]:
            invoke_notification_lambda("sms", row["phone_number"], message)

        return {"status": "success"}
    except Exception as e:
        logger.exception("send_order_status_notification error")
...
def _update_frequency_flags(customer_id: str, address_id: str,
                            pickup_service, dropoff_service) -> dict:
    """Update uberPickupFrequency / uberDropoffFrequency on laundry_frequency via PostgreSQL."""
    try:
        freq_row = get_frequency_by_address(customer_id, address_id)
        if not freq_row:
            return {"status": "skipped_no_frequencyId", "customerId": customer_id, "addressId": address_id}
        update_frequency_uber_flags(freq_row["frequency_id"], pickup_service, dropoff_service)
        return {"status": "updated", "frequencyId": freq_row["frequency_id"]}
    except Exception as e:
        logger.exception("_update_frequency_flags error")
        return {"status": "error", "message": str(e)}
...
def cancel_uber_delivery(
    ...
    auto_generated  = order_item.get("auto_generated")
    ord_customer_id = order_item.get("customer_id")
    ord_address_id  = order_item.get("address_id")
    ord_frequency   = order_item.get("frequency")

    # 3) Update pickup/dropoff service
    if pickup_service:
        update_order_service_field(order_id, "pickup_service", pickup_service)
    if dropoff_service:
        update_order_service_field(order_id, "dropoff_service", dropoff_service)
...
def handle_schedule_uber_order(event):
    try:
        ...
        response = get_order(external_id)
        order_item = response or {}

        # Store uber_info as JSONB in PostgreSQL
        import json as _json
        cur = db.get_cursor()
        existing_uber_info = order_item.get("uber_info") or {}
        existing_uber_info[type_order] = uber_info
        cur.execute("""
            UPDATE orders.orders SET uber_info = %s, updated_at = NOW()
            WHERE order_id = %s
        """, (_json.dumps(existing_uber_info), external_id))

        # Update pickup/dropoff service field
        service_field = "pickup_service" if type_order == "laundryPickup" else "dropoff_service"
        current_service = order_item.get(service_field.replace("_", "").replace("service", "_service"))
        if current_service != "Uber":
            cur.execute(f"UPDATE orders.orders SET {service_field} = 'Uber' WHERE order_id = %s", (external_id,))

        db.commit()

        # === Read order for frequency checks ===
        ord_item = get_order(external_id) or {}
        auto_generated  = ord_item.get("auto_generated", False)
        ord_freq        = ord_item.get("frequency")
        ord_customer_id = ord_item.get("customer_id")
        ord_address_id  = ord_item.get("address_id")
