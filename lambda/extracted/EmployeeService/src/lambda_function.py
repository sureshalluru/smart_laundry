...
def lambda_handler(event, context):
    try:
        ...
        elif operation == "getEmployeeReviews":
            laundry_id = query_params.get("laundryId")
            employee_id = query_params.get("employeeId")
            limit = int(query_params.get("limit", 5))  # Default to 5 most recent reviews
            return handle_get_employee_reviews(laundry_id, employee_id, limit)
...
def handle_create_employee(event):
    ...
    for employee_data in employees:
        try:
            ...
            emp_id = generate_emp_id(laundry_id)
...
def handle_get_employee_reviews(laundry_id, employee_id, limit=5):
    try:
        logger.info(f"Starting to process request for employee reviews. LaundryID: {laundry_id}, EmployeeID: {employee_id}, Limit: {limit}")

        if not laundry_id or not employee_id:
            error_msg = "Missing required parameters: laundryId and employeeId"
            logger.error(error_msg)
            raise ValueError(error_msg)

        # Create the partition key
        partition_key = f"{laundry_id}#{employee_id}"
        logger.debug(f"Generated partition key: {partition_key}")

        # Query DynamoDB for the employee's reviews, sorted by date (descending)
        logger.info(f"Querying DynamoDB for employee reviews with partition key: {partition_key}")
        response = reviews_table.query(
            KeyConditionExpression=Key('laundryId#empId').eq(partition_key),
            ScanIndexForward=False,  # Sort descending (newest first)
            Limit=limit
        )
        logger.debug(f"DynamoDB response received. Item count: {len(response.get('Items', []))}")

        # Process the items to extract the required fields
        reviews = []
        for item in response.get('Items', []):
            # Split the sort key to get orderDate and orderId
            sort_key_parts = item.get('orderDate#orderId', '').split('#')
            order_date = sort_key_parts[0] if len(sort_key_parts) > 0 else ''
            order_id = sort_key_parts[1] if len(sort_key_parts) > 1 else ''

            review = {
                'orderId': order_id,
                'reviewComment': item.get('reviewComment', ''),
                'employeeRating': float(item.get('employeeRating', 0)),  # Convert Decimal to float
                # 'photoUrl': item.get('photoUrl'),
                'reviewDate': item.get('reviewDate', '')
            }
            reviews.append(review)
            logger.debug(f"Processed review for order: {order_id}")

        logger.info(f"Successfully processed {len(reviews)} reviews")
        return {
            'statusCode': 200,
            'body': json.dumps({
                'message': 'Successfully retrieved employee reviews',
                'reviews': reviews
            }, default=str)  # Use default=str as fallback
        }

    except Exception as e:
        logger.error(f"Error occurred while retrieving employee reviews: {str(e)}", exc_info=True)
        return {
            'statusCode': 500,
            'body': json.dumps({
...
def generate_emp_id(laundry_id):
    try:
        # Fetch LaundryShopInfo for the laundry
        response = shop_info_table.get_item(Key={'laundryId': laundry_id})
        shop_info = response.get('Item', {})

        if not shop_info:
            raise ValueError(f"LaundryShopInfo not found for laundryId: {laundry_id}")

        # Try to get existing empPrefix
        prefix = shop_info.get("empPrefix")

        if not prefix:
            # Generate prefix from laundryName
            laundry_name = shop_info.get("laundryName", "LAU")
            prefix = laundry_name.replace(" ", "")[:3].upper()
            logger.info(f"Generated new prefix: {prefix}")

            # Save prefix back to LaundryShopInfo
            shop_info_table.update_item(
                Key={'laundryId': laundry_id},
                UpdateExpression="SET empPrefix = :p",
                ExpressionAttributeValues={':p': prefix}
            )
            logger.info(f"Saved prefix '{prefix}' to LaundryShopInfo for laundryId: {laundry_id}")
        else:
            logger.info(f"Found existing prefix: {prefix}")

        # Create final empId
        random_digits = random.randint(100, 999)
        emp_id = f"{prefix}{random_digits}"
        logger.info(f"Generated empId: {emp_id}")
        return emp_id

    except Exception as e:
        logger.exception("Failed to generate employee ID")
        raise
...
def generate_passcode():
    ...
    logger.debug(f"Generated 4-digit passcode: {passcode}")
    return passcode
