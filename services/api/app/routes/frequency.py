    ...
    try:
        with get_db() as conn:
            ...
            try:
                ...
                logger.warning(f"Auto-resume query skipped (columns may not exist): {e}")
    ...
    with get_db() as conn:
        ...
        for r in cur.fetchall():
            ...
            frequencies.append({
                ...
                "isPaused": r.get("is_paused") or False,
...
# ─── Customer Subscription Management (Simple) ───
...
async def pause_subscription(
    ...
    """Pause a frequency subscription. Sets is_paused = TRUE."""
    frequency_id = body.get("frequencyId")
    if not frequency_id:
        return {"status": "error", "message": "Missing frequencyId"}
    ...
    with get_db() as conn:
        ...
        cur.execute("""
            ...
            WHERE frequency_id = %s AND is_active = TRUE
        """, (frequency_id,))
        if cur.rowcount == 0:
...
async def resume_subscription(
    ...
    """Resume a paused frequency subscription."""
    frequency_id = body.get("frequencyId")
    if not frequency_id:
        return {"status": "error", "message": "Missing frequencyId"}
...
async def skip_subscription(
    ...
    """Skip the next pickup — advances future_pickup_date by one frequency interval."""
    frequency_id = body.get("frequencyId")
    if not frequency_id:
        return {"status": "error", "message": "Missing frequencyId"}
    ...
    with get_db() as conn:
        cur = get_cursor(conn)
        cur.execute("""
            SELECT frequency_id, frequency, future_pickup_date
    ...
    next_date = sub["future_pickup_date"] + timedelta(days=days)
    ...
    return {"status": "success", "message": f"Skipped. Next pickup: {next_date}", "nextPickupDate": str(next_date)}


@router.post("/subscription/change-date")
async def change_pickup_date(
    body: dict = Body({}),
    current_user: dict = Depends(get_current_user),
):
    """Change the next pickup date for a subscription."""
    frequency_id = body.get("frequencyId")
    new_date = body.get("newDate")  # format: YYYY-MM-DD

    if not frequency_id or not new_date:
        return {"status": "error", "message": "Missing frequencyId or newDate"}
    ...
    with get_db() as conn:
        ...
        cur.execute("""
            ...
            SET future_pickup_date = %s, updated_at = NOW()
            WHERE frequency_id = %s AND is_active = TRUE
        """, (new_date, frequency_id))
        if cur.rowcount == 0:
            return {"status": "error", "message": "Subscription not found"}

    return {"status": "success", "message": f"Pickup date changed to {new_date}"}
